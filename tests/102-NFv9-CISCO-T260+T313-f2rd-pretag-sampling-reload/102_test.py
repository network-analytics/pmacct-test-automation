...
def main(consumer):
    helpers.replace_in_file(testParams.results_log_files[0], '/etc/pmacct', testParams.pmacct_mount_folder, 'Reading configuration file')
    helpers.replace_in_file(testParams.results_log_files[1], '/etc/pmacct', testParams.pmacct_mount_folder)
    ...
    messages = consumer.get_messages(120, helpers.count_non_empty_lines(testParams.results_output_files[0])) # 35 lines
    ...
    helpers.replace_in_file(testParams.results_output_files[1], '192.168.100.1', '172.111.1.101')
    ...
    # Replaying traffic from folder pcap_mount_0 (traffic.pcap, traffic-reproducer.conf)
    assert scripts.replay_pcap_with_docker(testParams.results_pcap_folders[0], '172.111.1.101')
    messages = consumer.get_messages(120, helpers.count_non_empty_lines(testParams.results_output_files[1]))  # 35 lines
    assert messages != None and len(messages) > 0
    logger.info('Waiting 10 seconds')
    time.sleep(10)  # needed for the last message to exist in logs
